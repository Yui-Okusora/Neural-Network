...
namespace NeuroSoraCore
{
    ...
    {
        return m_values->refAt<float>((size_t)n_columns * row + col, view);
    }
    ...
    Matrix Matrix::setRandomVals(float randVal(void))
    {
        ...
        {
            m_values->refAt<float>(i, view) = randVal();
        }
        m_values->unload(view);
        return *this;
    }
    ...
    {
        Internal::Math::Mat::addVal2Matrix(this, b);
        return *this;
    }
    ...
    Matrix& Matrix::subtr_nocopy(const Matrix& b)
    {
        if (n_rows != b.getRows() || n_columns != b.getCols())
            throw std::out_of_range("Subtracting matrixes are not the same dimensions\n");
        Internal::Math::Mat::subtractMatrix(this, const_cast<Matrix&>(b));
        return *this;
    }

    Matrix Matrix::subtr(const Matrix& b)
    {
        Matrix tmp(*this);
        tmp.subtr_nocopy(b);
        return tmp;
    ...
    {
        Internal::Math::Mat::transposeMatrix(this);
    ...
    {
        Internal::Math::Mat::multiplyVal2Matrix(this, x);
        return *this;
    }
    ...
    Matrix& Matrix::multiply_nocopy(const Matrix& b)
    {
        if (n_rows != b.getRows() || n_columns != b.getCols())
            throw std::out_of_range("Scaling matrixes are not the same dimensions\n");
        Internal::Math::Mat::multiplyMat2Matrix(this, const_cast<Matrix&>(b));
        return *this;
    }
    ...
    Matrix& Matrix::dotProduct_nocopy(const Matrix& b)
    {
        Timer timer("Matrix::dotProduct_nocopy");
        if (n_columns != b.getRows())
            throw std::out_of_range("Multipling matrix A columns not equal matrix B rows\n");
        Internal::Math::Mat::dotProductMatrix(this, b);
        setCols(b.getCols());

        return *this;
    }

    Matrix Matrix::dotProduct(const Matrix& b)
    {
        Timer timer("Matrix::dotProduct");
        Matrix tmp(*this);
    ...
    {
        Internal::Math::Mat::applyActivationMatrix(this, activation, reluParam);
        return *this;
    }
    ...
    {
        Matrix tmp(*this);
        tmp.applyActivation_nocopy(activation, reluParam);
        return tmp;
    }

    Matrix& Matrix::applyActivationDerivative_nocopy(ActivationType activation, const Matrix* scalar, float reluParam)
    {
        if (scalar != NULL)
            if (n_rows != scalar->getRows() || n_columns != scalar->getCols())
                throw std::out_of_range("Matrixes are not the same dimensions\n");
        Internal::Math::Mat::applyActivationDerivativeMatrix(this, activation, reluParam, const_cast<Matrix*>(scalar));
        return *this;
    }

    Matrix Matrix::applyActivationDerivative(ActivationType activation, const Matrix* scalar, float reluParam)
    {
        Matrix tmp(*this);
    ...
    {
        Matrix tmp(*this);
        Internal::Math::Mat::rotateMatrix180(&tmp);
        return tmp;
    }
    ...
    float Matrix::sum()
    {
        ...
        for (unsigned i = 0; i < size; ++i)
        {
            sum += m_values->refAt<float>(i, view);
        }
        m_values->unload(view);
        return sum;
    }
